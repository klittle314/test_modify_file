...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$file1, {
                   ...
                   if(clinic_name %in% unique(df_master1$ClinicName)) {
                     ...
                     if(isTRUE(base::all.equal(nrec_clinic_old,nrec_clinic_new))) {
                     ...
                     } else if(nrec_clinic_old > nrec_clinic_new) {
                       ...
                       tbl_NA <- cbind(rep(clinic_name,nrec_clinic_old),matrix(data=NA,nrow=nrec_clinic_old,ncol=ncol_clinic-1))
                       ...
                       gs_edit_cells(ss=gsobj,ws="Summary_Data",input=df_clinic[1:nrec_clinic_old,],col_names=FALSE,anchor=anchor1)
                       #call the object again?
                       gsobj <- gs_key(x=gskey2)
                       anchor2 <- paste0("A", as.character(nrow(df_master1)+1))
                       gs_edit_cells(ss=gsobj,ws="Summary_Data",input=df_clinic[(nrec_clinic_old+1):nrec_clinic_new,],col_names=FALSE,anchor=anchor2)
                     }
